...
alpha textsets ast = runIdentity $ evalStateT (do
    mapM_ (\name -> do
...
alpha' (TopLevelLet (SimpleTypedVar t name) e) = do
    ...
    where
        ...
        alphaExpr (EThunk e) = EThunk <$> alphaExpr e
        ...
        alphaExpr e'@(Var (SimpleTypedVar t name)) = do
            ...
            case M.lookup name env of
                ...
                Nothing      -> pure e'
        alphaExpr (ExecThunk e) = ExecThunk <$> alphaExpr e
